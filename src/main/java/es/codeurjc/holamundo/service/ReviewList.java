...
public class ReviewList {
    ...
    private Map<Integer, Review> reviews = new HashMap<>(); // Map<bookID, Review>

    // Constructor
    public ReviewList() {
        // Example reviews for book with ID 1
        for (int i = 0; i < 18; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 1));
        }

        // Example reviews for book with ID 2
        for (int i = 18; i < 36; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 2));
        }

        // Example reviews for book with ID 3
        for (int i = 36; i < 54; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 3));
        }

        // Example reviews for book with ID 4
        for (int i = 54; i < 72; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 4));
        }

        // Example reviews for book with ID 5
        for (int i = 72; i < 90; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 5));
        }

        // Example reviews for book with ID 6
        for (int i = 90; i < 108; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 6));
        }

        // Example reviews for book with ID 7
        for (int i = 108; i < 126; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 7));
        }

        // Example reviews for book with ID 8
        for (int i = 126; i < 144; i++) {
            this.reviews.put(i, new Review(i, "Review " + i, "Author " + i, i, "Content " + i, 8));
        }
    ...
    public void updateReview(int bookID, int reviewID, Review review) {
    ...
    public Map<Integer, Review> getBookReviews(int bookID) {
        Map<Integer, Review> bookReviews = new HashMap<>();
